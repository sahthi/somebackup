...
#-------------------------------------------------------------------------------
#
# $Header: //components/rel/boot.bf/3.1.2.c2.2/boot_images/build/ms/build.sh#1 $
...
#9x45 compilation on nand, flashless and emmc
...
then
	...
	if [ $isProd -gt 0 ]
	then 
	  sh build_9x45.sh boot flashtools BUILD_ID=AAATANAZ $cmds  
	  sh build_9x45.sh boot BUILD_ID=AAAHANAZ $cmds
	  sh build_9x45.sh boot jsdcc deviceprogrammer BUILD_ID=AAASANAZ $cmds  
	else 
	  sh build_9x45.sh boot flashtools BUILD_ID=AAATANAA $cmds  
	  sh build_9x45.sh boot BUILD_ID=AAAHANAA $cmds
	  sh build_9x45.sh boot jsdcc deviceprogrammer BUILD_ID=AAASANAA $cmds  
	fi
   elif [[ $BUILD_ID =~ AAATANAZ ]] 
   then 
	sh build_9x45.sh boot flashtools BUILD_ID=$BUILD_ID $cmds  
   elif [[ $BUILD_ID =~ AAAHANAZ ]] 
   then
 	sh build_9x45.sh boot BUILD_ID=$BUILD_ID $cmds
   elif [[ $BUILD_ID =~ AAASANAZ ]] 
   then
 	sh build_9x45.sh boot jsdcc deviceprogrammer BUILD_ID=$BUILD_ID $cmds
   elif [[ $BUILD_ID =~ AAAT.. ]] 
   then 
	sh build_9x45.sh boot flashtools BUILD_ID=$BUILD_ID $cmds  
   elif [[ $BUILD_ID =~ AAAH.. ]] 
   ...
   then 
	...
	echo "Warning: build id required"
   fi
fi
...
then
	...
	then 
	  ...
	  sh build_8952.sh boot jsdcc deviceprogrammer BUILD_ID=SAASANAZ $cmds
	  ...
	  sh build_8952.sh boot jsdcc deviceprogrammer BUILD_ID=SAASANAA $cmds
   ...
   if [[ $BUILD_ID =~ SAAAANAZ ]] 
	...
	then
		#sh build_8952.sh boot jsdcc deviceprogrammer BUILD_ID=$BUILD_ID $cmds
		build_8952.sh boot jsdcc deviceprogrammer BUILD_ID=$BUILD_ID $cmds
...
if [ "$TARGET_FAMILY" = "8956" ]
then
    if [ -z $BUILD_ID ] 
    then
	if [ $isProd -gt 0 ]
	then 
	  sh build_8956.sh boot jsdcc deviceprogrammer BUILD_ID=EAAAANAZ $cmds
	  sh build_8956.sh boot jsdcc deviceprogrammer BUILD_ID=EAASANAZ $cmds  
	else 
          sh build_8956.sh boot BUILD_ID=EAAAANAA $cmds
          sh build_8956.sh boot BUILD_ID=EAASANAA $cmds
	fi
   else
   
   if [[ $BUILD_ID =~ EAAAANAZ ]] 
	then
		sh build_8956.sh boot jsdcc deviceprogrammer BUILD_ID=$BUILD_ID $cmds
	elif [[ $BUILD_ID =~ EAAAANAA ]] 
	then
		sh build_8956.sh boot BUILD_ID=$BUILD_ID $cmds
	elif [[ $BUILD_ID =~ EAASANAZ ]] 
	then
		build_8956.sh boot jsdcc deviceprogrammer BUILD_ID=$BUILD_ID $cmds
