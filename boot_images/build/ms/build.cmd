...
IF /I "%TARGET_FAMILY%" == "9x45" ( 
REM Default compilation on both flash and flashless   
  IF "%BUILD_ID%" == "" (
    IF %isProd% == 1 (  
    rem compiling prod build for 9x45
      CALL %BUILD_CMD% boot flashtools BUILD_ID=AAATANAZ %cmd%
      TYPE build-log.txt > buildlog-log.txt      
     CALL %BUILD_CMD% boot BUILD_ID=AAAHANAZ  %cmd% 
      TYPE build-log.txt >> buildlog-log.txt      
      CALL %BUILD_CMD% boot jsdcc deviceprogrammer BUILD_ID=AAASANAZ %cmd%
      TYPE build-log.txt >> buildlog-log.txt      
    )ELSE ( rem  normal flow/default 9x45 build
      CALL %BUILD_CMD% boot flashtools BUILD_ID=AAATANAA %cmd%
      TYPE build-log.txt > buildlog-log.txt
      CALL %BUILD_CMD% boot BUILD_ID=AAAHANAA %cmd%
      TYPE build-log.txt >> buildlog-log.txt
      CALL %BUILD_CMD% boot jsdcc deviceprogrammer BUILD_ID=AAASANAA %cmd%
      TYPE build-log.txt >> buildlog-log.txt
    )
  )ELSE (  rem compile base on Build ID
    goto  %BUILD_ID%
:AAATANAZ
   rem compiling AAATANAZ 9x45-prod nand
   CALL %BUILD_CMD% boot flashtools jsdcc BUILD_ID=%BUILD_ID% %cmd%
   goto END
:AAAHANAZ
   rem compiling aaahanaz 9x45-flashless prod
   CALL %BUILD_CMD% boot  BUILD_ID=%BUILD_ID% %cmd% 
   goto END
:AAASANAZ
   rem compiling AAASANAZ 9x45-prod emmc
   CALL %BUILD_CMD% boot jsdcc deviceprogrammer BUILD_ID=%BUILD_ID% %cmd%
   goto END
:AAATANAA
   rem compiling aaatanaa nand internal
   CALL %BUILD_CMD% boot flashtools jsdcc BUILD_ID=%BUILD_ID% %cmd%
   goto END
:AAAHANAA
   rem compiling aaahanaa flashless internal
   CALL %BUILD_CMD% boot  BUILD_ID=%BUILD_ID% %cmd% 
   goto END
:AAASANAA
   rem compiling aaasanaa emmc internal
   CALL %BUILD_CMD% boot jsdcc deviceprogrammer BUILD_ID=%BUILD_ID% %cmd%
   goto END
  )  
...
IF "%BUILD_ID%" == "" (
    IF %isProd% == 1 (  
    rem compiling prod build for 8952
      CALL %BUILD_CMD% boot jsdcc BUILD_ID=SAAAANAZ  %cmd% 
      TYPE build-log.txt >> buildlog-rumi.txt
	  CALL %BUILD_CMD% boot jsdcc deviceprogrammer BUILD_ID=SAASANAZ  %cmd% 
      ...
      rem CALL %BUILD_CMD% boot jsdcc deviceprogrammer BUILD_ID=SAAAANAA %cmd%
	  CALL %BUILD_CMD% boot jsdcc BUILD_ID=SAAAANAA %cmd%
	  TYPE build-log.txt >> buildlog-rumi.txt
	  CALL %BUILD_CMD% boot jsdcc deviceprogrammer BUILD_ID=SAASANAA %cmd%
...
:SAASANAA
   ...
   rem CALL %BUILD_CMD% boot jsdcc deviceprogrammer BUILD_ID=%BUILD_ID% %cmd%
   CALL %BUILD_CMD% boot jsdcc deviceprogrammer BUILD_ID=%BUILD_ID% %cmd%
   goto END
:SAAAANAA
   rem compiling saasanaa emmc internal
...
IF /I "%TARGET_FAMILY%" == "8956" ( 
IF "%BUILD_ID%" == "" (
    IF %isProd% == 1 (  
    rem compiling prod build for 8956
      CALL %BUILD_CMD% boot jsdcc deviceprogrammer BUILD_ID=EAAAANAZ  %cmd% 
      TYPE build-log.txt >> buildlog-rumi.txt  
 	  CALL %BUILD_CMD% boot jsdcc deviceprogrammer BUILD_ID=EAASANAZ  ?%cmd%
      TYPE build-log.txt >> buildlog-emmc.txt
    )ELSE ( rem  normal flow/default 8956 build
      rem CALL %BUILD_CMD% boot jsdcc deviceprogrammer BUILD_ID=EAAAANAA %cmd%
    ...
    goto  %BUILD_ID%
:EAAAANAZ
   rem compiling EAAAANAZ 8956-prod emmc
   CALL %BUILD_CMD% boot jsdcc deviceprogrammer BUILD_ID=%BUILD_ID% %cmd% 
   goto END
:EAASANAZ
   rem compiling EAASANAZ 8956-prod emmc
   CALL %BUILD_CMD% boot jsdcc deviceprogrammer BUILD_ID=%BUILD_ID% %cmd% 
   goto END
:EAAAANAA
