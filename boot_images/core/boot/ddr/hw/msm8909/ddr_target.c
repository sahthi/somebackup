...
$Header: //components/rel/boot.bf/3.1.2.c2.2/boot_images/core/boot/ddr/hw/msm8909/ddr_target.c#1 $
$DateTime: 2016/12/12 23:35:46 $
$Author: pwbldsvc $
================================================================================
when       who     what, where, why
--------   ---     -------------------------------------------------------------
06/10/16   sk      updated CA CDC delay value correctly.
...
/* =============================================================================
**  Function : HAL_SDRAM_PHY_Update_CA_CDC_Config_cdt
** =============================================================================
*/
/**
*   @brief
...
*   None
*/
void HAL_SDRAM_PHY_Update_CA_CDC_Config_cdt(uint32 base, SDRAM_INTERFACE interface, uint16 config)
{
  uint32 offset;
  uint32 phy_offset;  
...
} /* HAL_SDRAM_PHY_Update_CA_CDC_Config_cdt */

/* =============================================================================
**  Function : HAL_SDRAM_PHY_Update_DQ_CDC_Config_cdt
** =============================================================================
*/
/**
*   @brief
...
*   None
*/
void HAL_SDRAM_PHY_Update_DQ_CDC_Config_cdt(uint32 base, SDRAM_INTERFACE interface, uint32 dq_rd_wr, uint16 config[4])
{
  uint32 offset,dq;
  uint32 phy_offset;  
	 ...
	 for (dq = 0; dq < PHY_DQ_MAX_NUM; dq++)
		{
		 ...
		 BOOT_HWIO_OUTM(PHY_DQ_ADDR(WR_CDC_DELAY_CFG), offset,
                   ...
                   config[dq] << HWIO_SHFT(PHY_DQ_ADDR(WR_CDC_DELAY_CFG), DELAY_VAL));
		}
	}
	else
	{	 
	 for (dq = 0; dq < PHY_DQ_MAX_NUM; dq++)
		{
		 ...
		 BOOT_HWIO_OUTM(PHY_DQ_ADDR(RD_CDC_DELAY_CFG), offset,
                   ...
                   config[dq] << HWIO_SHFT(PHY_DQ_ADDR(RD_CDC_DELAY_CFG), DELAY_VAL));
		}
	}	
} /* HAL_SDRAM_PHY_Update_DQ_CDC_Config_cdt */
...
void ddr_target_init()
{
  ...
  ddr_func_tbl.phy_update_ca_cdc_config_cdt = (void (*)(uint32 , SDRAM_INTERFACE , uint16 )) 
                                    ((uint8 *)&HAL_SDRAM_PHY_Update_CA_CDC_Config_cdt - SCL_RPM_CODE_RAM_BASE);
  ddr_func_tbl.phy_update_dq_cdc_config_cdt = (void (*)(uint32 , SDRAM_INTERFACE , uint32, uint16 config[4])) 
                                    ((uint8 *)&HAL_SDRAM_PHY_Update_DQ_CDC_Config_cdt - SCL_RPM_CODE_RAM_BASE);
