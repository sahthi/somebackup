...
===========================================================================
  ...
  $Header: //components/rel/boot.bf/3.1.2.c2.2/boot_images/core/systemdrivers/clock/hw/msm8909/src/ClockSBL.c#2 $
  $DateTime: 2016/12/14 06:06:08 $
  $Author: pwbldsvc $
...
const boolean boot_debug_mode_enabled = TRUE;

#ifdef FEATURE_FORCE_DDR_TURBO_518_FOR_GPS
	  const boolean boot_ddr_turbo_518 = TRUE;
#else
	  const boolean boot_ddr_turbo_518 = FALSE;
#endif
...
{
  /* Enabled QUP4 I2C Apps clk (fixed at 19.2) and vote for the AHB */
...
static uint32 ddr_speed_khz = 400000;
...
boolean Clock_PreDDRInitEx( uint32 ddr_type )
{
  ...
  const ClockConfigMuxType clkCfg_ddr_debug = {HWIO_ADDR(GCC_BIMC_DDR_CMD_RCGR),  MUX_GCC, SRC_BIMCPLL,  2, 0, 0, 0};
  const ClockConfigMuxType clkCfg = {HWIO_ADDR(GCC_BIMC_DDR_CMD_RCGR),  MUX_GCC, SRC_GPLL0,  2, 0, 0, 0};
  ...
  if(boot_ddr_turbo_518==TRUE)
  {
       cfg->BIMCPLL_Cfg.nL = 0x36;
       cfg->BIMCPLL_Cfg.nAlpha = 0x0000000000;
  } 
  
  if(boot_debug_mode_enabled == TRUE)
  {
	/* MSM8909 boots DDR at turbo mode for DDI */

        if(boot_ddr_turbo_518==TRUE)
        {	 
             ddr_speed_khz = 518400;
        }
        else
        {
            /* MSM8905 boots DDR at 384 MHz */
	     ddr_speed_khz = 384000;
        }
  }
  else
  {
	/* MSM8909 boots DDR at 400 MHz */
	 ddr_speed_khz = 400000;
  } 
 ...
 /* Enable Imem and Msg RAM clock */
  ...
  if( boot_debug_mode_enabled == TRUE)
  {
    /* switch BIMC clock to turbo mode for DDI*/
 	if( ! Clock_EnableSource( clkCfg_ddr_debug.eSource )) return FALSE;
  }
  else
  {
  ...
  {
  	 /* switch BIMC clock to turbo mode for DDI*/
  	if( ! Clock_ConfigMux(&clkCfg_ddr_debug)) return FALSE;
  }
  else
  {
...
boolean Clock_ExitBoot(void)
{
  ...
  /* Disable I2C related clocks (Enabled in Clock_I2CInit)*/
  HWIO_OUTF(GCC_APCS_CLOCK_BRANCH_ENA_VOTE, BLSP1_SLEEP_CLK_ENA, 0);
  HWIO_OUTF(GCC_BLSP1_QUP4_I2C_APPS_CBCR, CLK_ENABLE, 0);
