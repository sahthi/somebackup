...
$Header: //components/rel/boot.bf/3.1.2.c2.2/boot_images/core/systemdrivers/pmic/target/msm8909_pm8909/system/src/pm_sbl_boot.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
11/21/16   kc      Update for the new CDT entreis for PM8916(CR-1092454)
08/05/16   kc      Added new Platform IDs support (CR-1047854)
12/21/15   mr      Added new Platform IDs support (CR-954225)
08/20/15   akt     Update for the new CDT entreis for PM8916 (CR-943917)
...
pm_err_flag_type pm_device_init ( void )
{
    ...
    if((DALPLATFORMINFO_TYPE_CDP == platformInfo.platform && 0x02 == platformInfo.subtype) ||
       ...
       (DALPLATFORMINFO_TYPE_RCM == platformInfo.platform && 0x03 == platformInfo.subtype) ||	   	   
