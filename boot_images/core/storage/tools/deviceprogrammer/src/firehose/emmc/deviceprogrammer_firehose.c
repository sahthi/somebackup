 ...
 *   order.
 *
 *   $Header: //components/rel/boot.bf/3.1.2.c2.2/boot_images/core/storage/tools/deviceprogrammer/src/firehose/emmc/deviceprogrammer_firehose.c#1 $ 
...
firehose_error_t handleErase() {
    ...
    while (xmlGetToken(&fh.xml_reader) == XML_TOKEN_ATTRIBUTE) {
        if (xmlIsAttribute(&fh.xml_reader, "storagedrive")) {
            xmlGetAttributeValue(&fh.xml_reader,
                                 temp_buffer,
                                 sizeof(temp_buffer));
            storage_drive_number = stringToNumber((const char *) temp_buffer, &num_conversion);
            if (FALSE == num_conversion) {
                logMessage("Failed to get storage drive number");
                response = NAK;
            }
        }
    }
    ...
    if (storage_drive_number == -1) {
        logMessage("No storage drive number");
        sendResponse(NAK);
        return FIREHOSE_ERROR;
    }
    if (FALSE == storage_device_open_partition(&fh.emmc_storage,
