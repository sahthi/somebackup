...
Copyright (c) 2003-2013,2014 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
/*===========================================================================
  ...
  $Header: //components/rel/boot.bf/3.1.2.c2.2/boot_images/core/storage/tools/deviceprogrammer/src/firehose/nand/deviceprogrammer_security.c#1 $ 
...
uint32 getChipSerialNum(void)
{
    ...
    if (serialNum == 0)
    {
        // changed nand deviceprogrammer
        //err = secboot_hw_get_serial_num(&serialNum);
        
		err = E_SECBOOT_HW_SUCCESS;
		serialNum = 1;
...
*/
	return FALSE; // HACK, don't know the function for 9x45 yet
}
...
boolean AuthenticateImage(byte* src_ptr)
{
    ...
    memset(&verified_info, 0, sizeof(secboot_verified_info_type));
