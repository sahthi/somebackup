 ...
 * $Header: //components/rel/boot.bf/3.1.2.c2.2/boot_images/core/buses/spmi/src/core/hal/bear/PmicArb.c#1 $
 * $Change: 11981336 $ 
 * 
 *                              Edit History
 * Date     Description
 * -------  -------------------------------------------------------------------
 * 7/20/15  KW error fixes.
...
Spmi_Result PmicArb_AddCacheEntry(uint32 chan, uint8 sid, uint8 periph)
{
    ...
    if(newEntry->sid != INVALID_SID) 
    {
        ...
        else
        {
            ...
            uint8 idx = (uint8)(newEntry->prevIdx);
