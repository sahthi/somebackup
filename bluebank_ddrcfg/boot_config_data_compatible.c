...
#define DESCRITE_LPDDR3_DETECT_PIN 88
  uint32 	status = 0;
  ...
  gpio_tlmm_config(DESCRITE_LPDDR3_DETECT_PIN, 0, GPIO_INPUT, GPIO_PULL_UP, GPIO_2MA, GPIO_ENABLE);
  busywait(1000);
  status = gpio_status(LPDDR3_DETECT_PIN);
  if(status == 0){
  ...
  else
  {
		...
		status = gpio_status(DESCRITE_LPDDR3_DETECT_PIN);
		 if(status == 0){
			boot_log_message("DESCRITE 88 PULL DOWN: L");
			config_data_table_info.size = config_data_table_size_lpddr3;
			config_data_table_info.cdt_ptr = config_data_table_lpddr3;
			boot_log_message("USE LPDDR3 parameter");
		}
